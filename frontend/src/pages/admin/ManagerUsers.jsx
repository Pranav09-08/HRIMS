...
function ManagerUsersPage() {
  ...
  const fetchRows = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/admin/users?role=manager`, {
        headers: authHeaders,
      })
      const payload = await response.json()

      if (!response.ok) {
        throw new Error(payload?.message || 'Failed to fetch manager users.')
      }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.message || 'Failed to save manager user.')
      }

      setSuccessMessage(isEdit ? 'Manager user updated successfully.' : 'Manager user created successfully.')
      resetForm()
      await fetchRows()
    } catch (error) {
      setErrorMessage(error.message)
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this manager user?')) {
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.message || 'Failed to delete manager user.')
      }
      ...
      setSuccessMessage('Manager user deleted successfully.')
      await fetchRows()
    } catch (error) {
      setErrorMessage(error.message)
    }
  ...
  return (
    <DashboardLayout role="admin" title="Admin Dashboard" subtitle="People: Manager">
      <div className="space-y-6">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.35em] text-black/40">People</p>
          <h2 className="mt-3 text-2xl font-semibold text-black">Manager</h2>
          <p className="mt-2 text-sm text-black/60">Manage manager users with full CRUD actions.</p>
        </div>

        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <section className="rounded-2xl border border-black/10 bg-[#fafafa] p-4">
            <h3 className="text-lg font-semibold text-black">{editingId ? 'Edit Manager User' : 'Create Manager User'}</h3>
            <p className="mt-1 text-sm text-black/55">Maintain manager accounts and update profile access quickly.</p>
          ...
          <section className="rounded-2xl border border-black/10 bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 className="text-lg font-semibold text-black">Manager Directory</h3>
              ...
              <table className="min-w-full divide-y divide-black/10 text-sm">
                ...
                <tbody className="divide-y divide-black/5">
                  {loading ? (
                    ...
                    <tr><td colSpan={5} className="px-4 py-4 text-black/50">No manager users found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
